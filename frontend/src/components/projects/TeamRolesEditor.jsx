...
const TeamRolesEditor = ({ teamRoles, setTeamRoles }) => {
  const [newUserId, setNewUserId] = useState(null);  // Estado para almacenar el ID del nuevo miembro
  const [newUserRole, setNewUserRole] = useState('');  // Estado para almacenar el rol del nuevo miembro
  ...
  const handleRemoveMember = async (userId) => {
    try {
      ...
      // Opcional: Actualizar el estado local después de eliminar
      setTeamRoles(teamRoles.filter(role => role.user_id !== userId));
